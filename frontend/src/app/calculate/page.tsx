...
const ShippingCalculator: React.FC = () => {
  ...
  const calculateCost = () => {
    ...
    let effectiveWeight = Math.max(weight, volumeWeight);
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      ...
      <div className={`grid grid-cols-3 gap-4 mb-4 ${calculateBy === "weight" ? "hidden" : "block" }`}>
        <div>
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-4">
          <button
            ...
            className={`px-4 py-2 rounded ${calculateBy === 'weight' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            Жингээр
          </button>
          <button
            onClick={() => setCalculateBy('volume')}
            className={`px-4 py-2 rounded ${calculateBy === 'volume' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            Овроор бодох
          </button>
        </div>
      ...
      {result !== null && (
        <div className="p-4 bg-gray-100 rounded">
          ...
          <p className="text-xl">{result } {calculateBy === 'weight' ? '¥' : '₮'}</p>
        </div>
      )}
      ...
      <div className="mt-6 text-sm text-gray-600">
        ...
        <ul className="list-disc pl-5">
          ...
          <li>Тусдал ирсэн болон бусад дамжуулалтын барааг тус тусдад нь бодно</li>
          <li>Овор ихтэй ууттай куртик, хүнжил гэх мэт ауулын овор хэмжээнээс хамаараад ойролцоогоор 5000₮–10000₮ хооронд байдаг</li>
          <li>Гутлын хайрцаг 4000₮–8000₮ хооронд</li>
          <li>Тээврийн зардал нь тухайн барааны жин болон "Оврын жин" хоёрын аль ихээр нь тооцно</li>
        </ul>
      </div>
    </div>
  );
